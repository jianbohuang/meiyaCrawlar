...
#获取关键字地址
def get_url():
    data = xlrd.open_workbook('D:/needfile/workspace/meiya4_try2.xlsx')  # 打开xlsx文件
    table = data.sheets()[0]  # 打开第一张表
    nrows = table.nrows  # 获取表的行数
    a = []
    for i in range(nrows):  # 循环逐行读取
...
def getcompanyinfoByDriver(account):
    ...
    for elem in account:
        ...
        for elem in a:
            ...
            page_index = elem.index('&ee=')
            key_url = elem[:page_index]
            page = elem[page_index+4:]
            ...
            print page
            while flag:
                url = str(key_url) + '&ee=%s' % page
                page += 1
                print url
                #driver.get(str(url))
                #如果是验证码页面则连续访问3次，连续失败后返回到失败列表
                try:
                    for i in xrange(4):
                        driver.get(str(url))
                        # time.sleep(1)
                        ...
                        #追加模式打开把失败连接加入文本文件
                        f = open("D:/needfile/workspace/fail_final.txt", 'ab')
                        f.write(str(url))
                        f.write('\r\n')
                        f.close()
                except:
